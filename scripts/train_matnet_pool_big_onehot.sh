...
# using deterministic one-hot init embeddings (better for probing / transfer analysis).
#
# This matches `scripts/train_matnet_pool_big.sh` (the `matnet_seattle_big` settings),
# but explicitly sets `--init_embedding_mode onehot`.
#
# Usage:
#   POOL_DIR=data/osm_pools/seattle_time_k10000_seed0 RUN_NAME=matnet_seattle_big_onehot bash scripts/train_matnet_pool_big_onehot.sh
#
# You can override any env var below.

RUN_NAME="${RUN_NAME:-matnet_seattle_pool_big_onehot_$(date +%Y%m%d_%H%M%S)}"
...
# Deterministic init embeddings (critical for consistent cross-instance probing)
INIT_EMBEDDING_MODE="${INIT_EMBEDDING_MODE:-onehot}" # onehot|random_onehot|random
