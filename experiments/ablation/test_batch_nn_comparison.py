...
def test_batch_nn_comparison():
    """Test batch comparison of multiple features using NN agreement rate CHANGE"""
    ...
    # Test parameters
    num_features = 10
    features_to_test = list(range(num_features))  # Features 0-9
    batch_size = 8  # Test with multiple instances
    test_steps = [0, 5, 10, 15]  # Test at multiple trajectory points
    
    print(f"Testing {num_features} features with batch size {batch_size}")
    print(f"Testing at trajectory steps: {test_steps}")
    print(f"Key insight: Measuring CHANGE in NN agreement (baseline - ablated)")
    
    # Create FIXED test instances that will be reused for all comparisons
    ...
    print(f"\n=== Testing Each Feature (N={num_features} forward passes) ===")
    ...
    print(f"Average per-feature time: {total_ablation_time/num_features:.3f}s")
    print(f"Total time: {baseline_time + total_ablation_time:.3f}s")
    print(f"Efficiency: {num_features+1} forward passes vs {2*num_features} (naive)")
    ...
    for step_idx, step in enumerate(test_steps):
        ...
        if baseline_step_vals and ablated_step_vals:
            ...
            print(f"  Step {step}: {avg_change:+.3f} average change across all features")
    
    print("\nBatch NN comparison test complete!")
    print("Key insight: Positive change = feature promotes NN behavior, Negative = feature inhibits NN behavior")
    return feature_results

if __name__ == "__main__":
    results = test_batch_nn_comparison() 